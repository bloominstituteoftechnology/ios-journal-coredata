...
class EntriesTableViewController: UITableViewController {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "AddEntrySegue" {
            ...
            guard let showDetailVC = segue.destination as? EntryDetailViewController, let indexPath = tableView.indexPathForSelectedRow else { return }
            showDetailVC.entryController = entryController
            showDetailVC.entry = fetchedResultsController.object(at: indexPath)
        }
    }
}
