...
class EntriesTableViewController: UITableViewController {
    ...
    lazy var fetchedResultsController: NSFetchedResultsController<Entry> = {
        ...
        let coreMainContext = CoreDataStack.shared.mainContext
        let fetchResults = NSFetchedResultsController(fetchRequest: fetchRequest,
                                             managedObjectContext: coreMainContext,
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        guard let cell = tableView.dequeueReusableCell(withIdentifier: EntryTableViewCell.reuseIdentifier, for: indexPath) as? EntryTableViewCell else {
            fatalError("Could not dequeue cell with name \(EntryTableViewCell.reuseIdentifier)")
        }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "EntryDetailsViewController" {
